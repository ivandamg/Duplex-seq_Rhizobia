...
Extract unmapped reads
          ...
          for FILE in $(ls *_L*_bwa-mem2_Medicago_Sorted.bam); do echo $FILE; sbatch --partition=pall --job-name=$(echo $FILE | cut -d'_' -f1,2)ST --time=0-03:00:00 --mem-per-cpu=64G --ntasks=2 --cpus-per-task=1 --output=$(echo $FILE | cut -d'_' -f1,2)_ST.out --error=$(echo $FILE | cut -d'_' -f1,2)_ST.error --mail-type=END,FAIL --wrap "module load UHTS/Analysis/samtools/1.10; cd /data/projects/p495_SinorhizobiumMeliloti/02_DuplexSeq/03_Mapped_Medicago/; samtools view -b -f 4 $FILE > $(echo $FILE | cut -d'.' -f1)_MedicagoUnmapped.bam; sh ~/00_Software/bedtools2/bin/bamToFastq -i $(echo $FILE | cut -d'.' -f1)_MedicagoUnmapped.bam -fq $(echo $FILE | cut -d'.' -f1)_MedicagoUnmapped.1.fq -fq2 $(echo $FILE | cut -d'.' -f1)_MedicagoUnmapped.2.fq; gzip $FILE "; sleep 1; done
