...
ex. https://www.biostars.org/p/279951/
            ...
            for FILE in $(ls Argon1_L1*Sorted_NewName.bam); do echo $FILE; sbatch --partition=pall --job-name=$(echo $FILE | cut -d'_' -f1,2)ST --time=0-03:00:00 --mem-per-cpu=64G --ntasks=2 --cpus-per-task=1 --output=$(echo $FILE | cut -d'_' -f1,2)_ST.out --error=$(echo $FILE | cut -d'_' -f1,2)_ST.error --mail-type=END,FAIL --wrap "conda activate rhizo; module load UHTS/Analysis/samtools/1.10; cd /data/projects/p495_SinorhizobiumMeliloti/02_DuplexSeq/10_fastp/; umi_tools group -I $FILE --paired --group-out=$(echo $FILE | cut -d'_' -f1,2)_grouped.tsv --output-bam -S $(echo $FILE | cut -d'_' -f1,2)_deduplicated_TAGGED.bam --read-length -L $(echo $FILE | cut -d'_' -f1,2)_stats.txt"; sleep 1; done
